...
result_t YDlidarDriver::grabScanData(node_info *nodebuffer, size_t &count, uint32_t timeout) {
	/*
	 * The receiver buffer is filled faster than
	 * the software is able to process them.
	 * So, we discard old data and wait for fresh
	 * data every time grabScanData is called.
	 */
  	_serial->flushInput();

	return waitScanData(nodebuffer, count, timeout);
}
